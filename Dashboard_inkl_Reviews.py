...
st.write("Cache Pfad:", CACHE_FILE_PATH)


def main():
    #Cache einmal pro Session laden
    ...
    with row1_left:
        ...
        with a:
            plz_input = st.text_input("PLZ", placeholder="PLZ eintragen").strip()
        with b:
    ...
    # --- WICHTIG: Streamlit rerunnt das ganze Script bei jedem UI-Klick (auch Checkbox).
    # Wenn wir hier bei do_search == False einfach returnen, verschwindet die Ergebnistabelle.
    # Lösung: Letztes Suchergebnis im session_state speichern und bei späteren Reruns wiederverwenden.
    ...
    # --- Nur nach erfolgreicher Suche: Ergebnis speichern, damit Checkbox-Reruns nicht alles zurücksetzen
    if do_search:
    ...
    shown_cols = ["Handwerker_Name", "PLZ_HW", "Land", "Entfernung in km (txt)",
            ...
            "Google Reviews", "Google Reviews laden"]
    ...
    # --- Checkbox soll SOFORT laden (ohne extra Button) ---
    # Hinweis: Checkbox-Klick triggert einen Streamlit-Rerun -> wir sind jetzt genau in diesem Rerun.
    # Wir lesen die angehakten Zeilen aus "edited" aus und laden direkt die Reviews nach.
    # WICHTIG:
    
    # Defensiv: Spalte existiert evtl. noch nicht (z.B. erster Run / Rerun)
    # if "Google Reviews laden" in edited.columns:
    #     to_load = edited.loc[
    #         edited["Google Reviews laden"] == True,
    #         ["Handwerker_Name", "PLZ_HW", "Land"]
    #     ].drop_duplicates()
    # else:
    #     to_load = pd.DataFrame(columns=["Handwerker_Name", "PLZ_HW", "Land"])
    ...
    #     # Optionaler Spinner: der User sieht kurz "Lade..." während der API Call läuft
    #     with st.spinner("Lade Google Reviews …"):
    #         for _, r in to_load.iterrows():
    #             try:
    #                 # Diese Funktion aktualisiert df_cache und speichert (in GooglePlaces) auch in cache.csv
    #                 _, df_cache = get_handwerker_data(
    ...
    #                 )
    #             except Exception as e:
    #                 # Keine st.success()/st.warning() hier, weil du "nichts passieren" willst.
    #                 # Bei Bedarf könntest du hier st.toast(...) nutzen.
    #                 had_error = True
    #                 st.error(f"Google Reviews Fehler für {r['Handwerker_Name']}: {e}")

    #     # Cache in der Session aktualisieren
    #     st.session_state.google_cache = df_cache

    #     # WICHTIG: Wir rerunnen, damit oben "Google Reviews aus Cache vorbelegen"
    #     # sofort das neue Rating im Format "rating (anzahl)" zeigt.
    #     st.rerun()
