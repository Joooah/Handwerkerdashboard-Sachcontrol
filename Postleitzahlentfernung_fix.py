...
def build_auftrag_geo_from_df(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, BallTree]:
    # Note: keep behaviour compatible with existing code: load data inside function
...
def datensaetze_im_umkreis(input_plz: str, radius_km: float, country: str, auftrag_geo: pd.DataFrame, plz_coords: pd.DataFrame,tree: BallTree,) -> pd.DataFrame:
    ...
    # Run the spatial index search (global tree may contain PLZ from multiple countries)
    ...
    # Merge with auftrag_geo to get matching Handwerker entries
    result = auftrag_geo.merge(nearby_pairs, on=["Land", "PLZ_HW"], how="inner")

    if not result.empty:
        # Compute accurate haversine distances (in km) and filter strictly by radius_km
        ...
        # ENSURE: only keep entries that are truly within the requested radius
        result = result[result["Entfernung_km"] <= float(radius_km)].copy()
